...
try:
    ...
    targets = []
    for box_idx in range(1, 66):  # 'A1'부터 'A5'까지 순회
...
for i in range(num_screenshots):
    ...
    for target in targets[:]:  # targets 리스트를 복사하여 순회
        ...
        if found:
            ...
            # "close.png" 이미지 클릭
            for _ in range(5):  # 여러 번 시도해서 이미지를 찾음
                ...
                if close_image_location:
                    ...
                    print("Close button image not found on the screen, retrying...")
                    time.sleep(1)  # 1초 대기 후 재시도
            else:
                print("Close button image not found on the screen after multiple attempts.")
