...
def main():
    ...
    # 이미지 영역 선택 및 클릭 횟수 입력 반복
    for i in range(8):
        print(f"{i+1} 번째 이미지를 선택하세요.")
        area = get_selection_area()
        image_path = f"temp_screenshot{i+1}.png"
        capture_and_save_area(area, image_path)
        images_list.append(image_path)

        # 이미지 클릭 횟수를 사용자로부터 입력받음
        click_count = int(input(f"{i+1} 번째 이미지를 몇 번 클릭하시겠습니까? "))
        # 이미지와 관련된 정보를 저장
        images_list.append((area, image_path, click_count))

    # 반복 실행
    for i in range(repeat_count):
        print(f"\n반복 {i+1}/{repeat_count}")
        for index, (area, image_path, click_count) in enumerate(images_list, start=1):
